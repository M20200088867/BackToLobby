...
function LoginContent() {
  ...
  const { session, isLoading, configured, signInWithGoogle } = useAuthContext();
  ...
  return (
    <div className="glass p-8 rounded-3xl max-w-md w-full text-center space-y-6">
      ...
      <p className="text-muted-foreground">
        Sign in with Google to start your gaming diary.
      </p>
      ...
      {error === "not_configured" && (
        <div className="flex items-center gap-2 text-sm text-yellow-400 bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-3">
          ...
          <span>Supabase is not configured. Set environment variables to enable auth.</span>
        </div>
      )}
